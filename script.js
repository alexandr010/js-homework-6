 ...
 */
 function checkEmail(email) {
   return(/^[a-zA-z]+\W?[a-zA-z]+@[a-zA-z\.]+\.[a-z]{2,3}$/).test(email);
 }
 ...
 */
 function checkDomainUrl(url) {
   return (/^(http:|https:)\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}?$/).test(url)
 }
...
function createLinksFromDomains(url) {
  ...
  url.match(reg).forEach((item) => url = url.replace(item, `<a href="${item}" target="_blank">${item.replace(/^(http:|https:)\/\//gi, '')}</a>`));
